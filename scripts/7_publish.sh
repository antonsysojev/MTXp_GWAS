#!/user/bin/env bash
### LAST VERSUPN UPDATE AUG 24 2023 (v1.1) - GOT RID OF VARIOUS BUGS...
...
for COHORT in P1YR P3YR; do
	...
	#mv METAANALYSIS1.TBL ${TMP}/RA_META_${COHORT}.tsv; rm METAANALYSIS1.TBL.info
...
for GWAS in RA_P1YR RA_P3YR RA_META_P1YR RA_META_P3YR SPOS_P1YR SPOS_P3YR SNEG_P1YR SNEG_P3YR; do
	...
	cp ${TMP}/${GWAS}.tsv .
	md5sum ${GWAS}.tsv > data/public/${GWAS}.md5sum
	tar -czvf data/public/${GWAS}.tsv.tar.gz ${GWAS}.tsv
	rm ${GWAS}.tsv
...
# 1.2. The zipping is probably not that efficient (I had an issue where the full path was being zipped up) but this works for now... perhaps the md5sum file should go into the zipped folder though? That's for later...
### NOTES:
